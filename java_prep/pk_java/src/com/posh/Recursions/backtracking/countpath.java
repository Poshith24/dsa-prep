...
//        maze[1][1] =true;
        System.out.println(allpaths(0,0,"",maze));
    }
...
//  river as an obstacle.
    ...
    static List<String> allpaths(int r, int c, String str,boolean[][] maze){
        ...
        List<String> list = new ArrayList<>();
        if(r<maze.length-1 && !maze[r+1][c]){
            if(!(r+1==maze.length-1 && c==maze[0].length-1)){
                maze[r+1][c]=true;
            }
            list.addAll(allpaths(r+1,c,str+"D",maze));
        }
        if(c<maze[0].length-1 && !maze[r][c+1]){
            if(!(r==maze.length-1 && c+1==maze[0].length-1)){
                maze[r][c+1]=true;
            }
            list.addAll(allpaths(r,c+1,str+"R",maze));
        }
        if(c>0 && !maze[r][c-1]){
            if(!(r==maze.length-1 && c-1==maze[0].length-1)){
                maze[r][c-1]=true;
            }
            list.addAll(allpaths(r,c-1,str+"L",maze));
        }
        if(r>0 && !maze[r-1][c]){
            if(!(r-1==maze.length-1 && c==maze[0].length-1)){
                maze[r-1][c]=true;
            }
            list.addAll(allpaths(r-1,c,str+"U",maze));
        }
